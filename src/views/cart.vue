<template>
	<div class="cart">
	    <div class="cart-header">
			...
			<Button type="primary" @click="modal1 = true" class="net-easy-Cloud">网易云</Button></div>
			<Modal
			    v-model="modal1"
			    title="搜索网易云"
			    @on-ok="ok"
			    @on-cancel="cancel">
				<div>
					<Input v-model="netEasyIDmusic" placeholder="输入六位数字ID查询 例:222222" style="width: 300px" />
					<Button type="primary" :loading="loading" @click="toLoading" shape="circle">
					    <span v-if="!loading">OK</span>
					    <span v-else>Loading...</span>
					</Button>
				</div> 
			    <div>{{netEasyCloudData}}</div>
				 <Tabs value="name1">
				        <TabPane label="歌名" name="name1">{{name}}</TabPane>
				        <TabPane label="作家" name="name2">{{author}}</TabPane>
				        <TabPane label="链接" name="name3">{{url}}</TabPane>
				 </Tabs>
			</Modal>
	    ...
	    <div class="cart-content">
	        <div class="cart-content-main" v-for="(item, index) in cartList" :key="item.name">
	...
	export default{
		...
		methods: {
			toLoading(){
				...
				this.axios({
					method:'get',
					url:'https://bird.ioliu.cn/netease/song',
					params:{
						id:that.netEasyIDmusic
					}
				})
				.then(function (response) {
					let data = response.data.data;
				    // that.netEasyCloudData=data;
					that.loading=false
					that.name=data.name
					that.author=data.ar[0].name
					that.url=data.al.picUrl
				  })
		...
		mounted () {
		  ...
		  for(let i =0;i<aim.length;i++){
			  ...
			  // aim[i].children[0].children[0].src=this.productDictList[this.cartList[i].id].image
			  let imgurl = this.productDictList[this.cartList[i].id].image;
			  const name = imgurl.split('/')[2]
			  const nameRes = './'+name;
			  aim[i].children[0].children[0].src = context(nameRes)
		  }
